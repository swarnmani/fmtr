...
/**
 * @brief Select Analog/Digital mode 
 */
typedef enum
{
    ADMODE_Analog = 0,  /** Analog input mode */
    ADMODE_Digital,     /** Digital functional mode */    
}ADMODE_Typedef;


#define PARAM_ADMODE(n)  ((n==ADMODE_Analog) || \
                           (n==ADMODE_Digital))

#define PARAM_ADCHANNEL(n) ((n>=0) && (n<=7))

#define IOCON_ADMODE_MASK     ((uint32_t)0x1<<7)
...
void IOCON_SetADMode(uint32_t pin, ADMODE_Typedef mod);                 
