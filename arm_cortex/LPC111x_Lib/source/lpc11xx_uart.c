...
Status uart_set_divisors(UART_TypeDef *UARTx, uint32_t baudrate)
{
	...
	* 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
	...
	{
    	((UART_TypeDef *)UARTx)->LCR |= UART_LCR_DLAB_EN;
    	((UART_TypeDef *)UARTx)->DLM = UART_LOAD_DLM(diviserOptimal);
    	((UART_TypeDef *)UARTx)->DLL = UART_LOAD_DLL(diviserOptimal);
    	/* Then reset DLAB bit */
    	((UART_TypeDef *)UARTx)->LCR &= (~UART_LCR_DLAB_EN) & UART_LCR_BITMASK;
    	((UART_TypeDef *)UARTx)->FDR = (UART_FDR_MULVAL(mulFracDivOptimal) \
...
void UART_Init(UART_TypeDef *UARTx, UART_CFG_Type *UART_ConfigStruct)
{
	...
	/* FIFOs are empty */
	((UART_TypeDef *)UARTx)->FCR = ( UART_FCR_FIFO_EN \
			| UART_FCR_RX_RS | UART_FCR_TX_RS);
	// Disable FIFO
	((UART_TypeDef *)UARTx)->FCR = 0;

	// Dummy reading
	while (((UART_TypeDef *)UARTx)->LSR & UART_LSR_RDR)
	{
		tmp = ((UART_TypeDef *)UARTx)->RBR;
	}

	((UART_TypeDef *)UARTx)->TER = UART_TER_TXEN;
	// Wait for current transmit complete
	while (!(((UART_TypeDef *)UARTx)->LSR & UART_LSR_THRE));
	// Disable Tx
	((UART_TypeDef *)UARTx)->TER = 0;

	// Disable interrupt
	((UART_TypeDef *)UARTx)->IER = 0;
	// Set LCR to default state
	((UART_TypeDef *)UARTx)->LCR = 0;
	// Set ACR to default state
	((UART_TypeDef *)UARTx)->ACR = 0;
	// Set Modem Control to default state
	((UART_TypeDef *)UARTx)->MCR = 0;
	// Set RS485 control to default state
	((UART_TypeDef *)UARTx)->RS485CTRL = 0;
	// Set RS485 delay timer to default state
	((UART_TypeDef *)UARTx)->RS485DLY = 0;
	// Set RS485 addr match to default state
	((UART_TypeDef *)UARTx)->ADRMATCH = 0;
	//Dummy Reading to Clear Status
	tmp = ((UART_TypeDef *)UARTx)->MSR;
	tmp = ((UART_TypeDef *)UARTx)->LSR;
		
	// Set Line Control register ----------------------------

	uart_set_divisors(UARTx, (UART_ConfigStruct->Baud_rate));

	tmp = (((UART_TypeDef *)UARTx)->LCR & (UART_LCR_DLAB_EN | UART_LCR_BREAK_EN)) \
			& UART_LCR_BITMASK;
	...
	((UART_TypeDef *)UARTx)->LCR = (uint8_t)(tmp & UART_LCR_BITMASK);	
}
...
void UART_SendData(UART_TypeDef* UARTx, uint8_t Data)
{
	...
	((UART_TypeDef *)UARTx)->THR = Data & UART_THR_MASKBIT;
}
...
uint8_t UART_ReceiveData(UART_TypeDef* UARTx)
{
	...
	return (((UART_TypeDef *)UARTx)->RBR & UART_RBR_MASKBIT);
}
...
void UART_ForceBreak(UART_TypeDef* UARTx)
{
	...
	((UART_TypeDef *)UARTx)->LCR |= UART_LCR_BREAK_EN;
}
...
//			break;
	...
	if (NewState == ENABLE)
	{		
		((UART_TypeDef *)UARTx)->IER |= tmp;
	}
	else
	{		
		((UART_TypeDef *)UARTx)->IER &= (~tmp) & UART_IER_BITMASK;
	}
}
...
//	}
	...
	((UART_TypeDef *)UARTx)->FCR = tmp & UART_FCR_BITMASK;	
}
				...
				FunctionalState NewState)
{
	...
	if (NewState == ENABLE)
	{
		...
		((UART_TypeDef *)UARTx)->LCR |= UART_LCR_DLAB_EN;
		((UART_TypeDef *)UARTx)->DLL = 0;
		((UART_TypeDef *)UARTx)->DLM = 0;
		((UART_TypeDef *)UARTx)->LCR &= ~UART_LCR_DLAB_EN;
		// FDR value must be reset to default value
		((UART_TypeDef *)UARTx)->FDR = 0x10;
		((UART_TypeDef *)UARTx)->ACR = UART_ACR_START | tmp;
	}
	else
	{
		((UART_TypeDef *)UARTx)->ACR = 0;
...
void UART_TxCmd(UART_TypeDef *UARTx, FunctionalState NewState)
{
	...
	{
		((UART_TypeDef *)UARTx)->TER &= (~UART_TER_TXEN) & UART_TER_BITMASK;
	}
}
 ...
 * @return none
 **********************************************************************/
void UART_FullModemForcePinState(UART_TypeDef *UARTx, UART_MODEM_PIN_Type Pin, \
 ...
 * @return none
 **********************************************************************/
void UART_FullModemConfigMode(UART_TypeDef *UARTx, UART_MODEM_MODE_Type Mode, \
							FunctionalState NewState)
{
	uint8_t tmp;
 ...
 * 			used to check all flags.
 **********************************************************************/
uint8_t UART_FullModemGetStatus(UART_TypeDef *UARTx)
{
	CHECK_PARAM(PARAM_UART_MODEM(UARTx));
 ...
 * @return		None
 **********************************************************************/
void UART_RS485Config(UART_TypeDef *UARTx, UART_RS485_CTRLCFG_Type *RS485ConfigStruct)
{
	uint32_t tmp;
 ...
 * @return		None
 */
void UART_RS485ReceiverCmd(UART_TypeDef *UARTx, FunctionalState NewState)
{
	if (NewState == ENABLE){
 ...
 * @return		None.
 */
uint32_t UART_RS485Send(UART_TypeDef *UARTx, uint8_t *pDatFrm, \
 ...
 * @return		None.
 */
void UART_RS485SendSlvAddr(UART_TypeDef *UARTx, uint8_t SlvAddr)
{
 ...
 * @return		None.
 */
uint32_t UART_RS485SendData(UART_TypeDef *UARTx, uint8_t *pData, uint32_t size)
{
